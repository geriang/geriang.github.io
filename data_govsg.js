...
async function loadCoordinate(month,year,streetName){
    let array = []
    const response = await axios.get(`${dataGovSGBaseApiUrl}${resourceID}&filters={"month":"${year}-${month}","street_name":"${streetName}"}&limit=1200`)
...
async function loadResalePrice(month,year,streetName){
    let array = []
    const response = await axios.get(`${dataGovSGBaseApiUrl}${resourceID}&filters={"month":"${year}-${month}","street_name":"${streetName}"}&limit=1200`)
    const record = response.data.result.records
    for (eachRecord of record){
